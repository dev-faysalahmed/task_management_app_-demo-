...
class ApiCaller{
  ...
  static Future<ApiResponse> getResponse({required String url})async{
    ...
    try {
      ...
      if(response.statusCode == 200){
        ...
        await _moveToLogin();
        return ApiResponse(isSuccess: false, responseCode: statusCode, responseData: null, errorMessage: 'Un-Authorized');
      }else{
        // Failed
        final decodedData = jsonDecode(response.body);
        return ApiResponse(isSuccess: false, responseCode: statusCode, responseData: decodedData, errorMessage: decodedData['data']);
      }
  ...
  static Future<void> _moveToLogin() async {
    ...
    Navigator.pushNamedAndRemoveUntil(TaskManagementApp.navigator.currentContext!, LoginScreen.name, (predicate)=>false);
  }
...
class ApiResponse{
  ...
  ApiResponse({required this.isSuccess, required this.responseCode, required this.responseData, this.errorMessage = 'Something went wrong'});
